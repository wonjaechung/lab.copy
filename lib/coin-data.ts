...
export type FilterCategoryId = "marketCap" | "changeRate" | "volume" | "category" | "staking" | "lending" | "circulatingRatio" | "athDrop" | "atlRise" | "streak" | "rsi" | "beta";
...
export const FILTER_GROUPS: FilterGroup[] = [
  { id: "basic", title: "기본 정보", icon: "info", categories: ["marketCap", "circulatingRatio", "category", "staking", "lending"] },
  { id: "price", title: "가격 & 거래", icon: "trending-up", categories: ["changeRate", "volume", "athDrop", "atlRise", "streak"] },
...
export const ALL_COINS: CoinItem[] = [
  ...
  { id: "xrp", name: "리플", symbol: "XRP", iconType: "feather", iconName: "send", iconColor: "#00AAE4", change: -0.8, change1h: -0.3, price: "852", marketCap: "42조", rank: 5, changeWeek: -2.1, changeMonth: 5.3, volume24h: "4.8조", rVol: 1.5, category: "layer1", tags: ["layer1", "rwa"], stakable: false, lendable: true, circulatingRatio: 56, athDrop: { all: 45, y1: 30, m6: 18, m3: 10 }, atlRise: { all: 280, y1: 50, m6: 25, m3: 12 }, streak: 0, rsi: 38, beta: 0.42 },
  ...
  { id: "sui", name: "수이", symbol: "SUI", iconType: "feather", iconName: "droplet", iconColor: "#4DA2FF", change: 12.3, change1h: 3.1, price: "4,850", marketCap: "12조", rank: 8, changeWeek: 18.5, changeMonth: 42.1, volume24h: "6.2조", rVol: 4.5, category: "layer1", tags: ["layer1", "layer2", "defi"], stakable: true, lendable: false, circulatingRatio: 28, athDrop: { all: 2, y1: 0, m6: 0, m3: 0, m1: 0 }, atlRise: { all: 1200, y1: 450, m6: 180, m3: 85, m1: 42 }, streak: 4, rsi: 78, beta: 1.65 },
...
export function filterCoins(filters: Record<FilterCategoryId, string | null>): CoinItem[] {
  return ALL_COINS.filter(coin => {
    ...
    if (filters.volume) {
      ...
      } else if (vf.startsWith("customRvol:")) {
        const threshold = parseFloat(vf.replace("customRvol:", ""));
        if ((coin.rVol ?? 0) < threshold) return false;
      } else if (vf.startsWith("rvol_")) {
        const mult = parseInt(vf.replace("rvol_", ""), 10) / 100;
    ...
    if (filters.streak) {
      const minDays = parseInt(filters.streak, 10);
